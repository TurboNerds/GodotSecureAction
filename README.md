...
| `godot-secure-tag` | `v1.0.2-alpha` | Release tag to download `godot_secure.py` from. Example: `v1.0.2-alpha`, `v1.2.0`. |
...
### Download the default Godot Secure release (v1.0.2-alpha)

`godot-secure-repo` defaults to `emabrey/Godot-Secure` and `godot-secure-tag` defaults to `v1.0.2-alpha`, so no extra inputs are needed to use the current stable release.
    ...
    steps:
      ...
      - name: Download Godot source and Godot Secure script
        ...
        with:
          ...
          # emabrey/Godot-Secure @ v1.0.2-alpha
          target: all
          scons-cache: true
          use-lto: true
...
4. **Downloads the Godot Secure release artifact** from `https://github.com/{godot-secure-repo}/releases/download/{godot-secure-tag}/{godot-secure-asset}` when `godot-secure-repo` is set. Defaults to the `godot_secure.py` asset from `emabrey/Godot-Secure` at `v1.0.2-alpha`.
